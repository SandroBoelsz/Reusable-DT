  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(
      input$run_simulation,
      {
        ...
        docker_call <- paste0("../back-end/scripts/cloud_execution.sh ", "/", run_dir)
        ...
        #docker_call <- paste0('docker run -v "', , '":"/scripts" -v "', Sys.getenv("R_PATH"), '":"/R" -v "', paste0(Sys.getenv("DATA_PATH"), stringr::str_remove(run_dir, paste0(Sys.getenv("HOME_PATH"), "shared"))), '":"/data" -e INPUT_DIR="/data" -e OUTPUT_DIR="/data/output" -e MAP="map.tif" -e LOOKUP_TABLE="lookup_table.csv" -e LOCATIONS="locations.csv" -e PARAMETERS="parameters.csv" -e NETLOGO_JAR_PATH="/NetLogo 6.3.0/lib/app/netlogo-6.3.0.jar" -e MODEL_PATH="/data/Beehave_BeeMapp2015_Netlogo6version_PolygonAggregation.nlogo" -e CPUS="1" --cpus 1 --platform linux/amd64 --entrypoint /scripts/run_docker_flow.sh ghcr.io/biodt/beehave:0.3.11')

        print(docker_call)
        system(docker_call)

        # Update output data ----
        new_out <- file.path(run_dir, "output_id1_iter1.csv")
